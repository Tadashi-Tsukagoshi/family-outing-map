...
import type { CollectedEvent } from './events'
...
/**
 * 承認済みイベントをSupabaseから取得する（サーバー専用）。
 * /api/events と同じ整形ロジックだが、既存ルートへの影響を避けるためこちらは独立した実装として持つ。
 */
export async function fetchApprovedEvents(): Promise<CollectedEvent[]> {
  ...
  return data.map((e): CollectedEvent => ({
    id: e.id,
    name: e.name,
    ...
    lng: e.lng,
    category: e.category,
    subCategory: e.sub_category ?? undefined,
    ...
    eventDates: Array.isArray(e.event_dates) && e.event_dates.length > 0
      ? (e.event_dates as EventDateRow[])
          ...
          .map(d => ({
            ...
            // このpage専用の取得では notice/画像の個別上書きは使わないため、常に親イベント側にフォールバックさせる
            useCustomNotice: false,
