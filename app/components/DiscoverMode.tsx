...
import { getDateDisplay, getEventStatus, STATUS_CONFIG, computeSpotDateRange } from '@/lib/date-utils'
...
function DiscoverCard({ spot, userLocation, onOpenDetail }: { spot: Spot; userLocation: [number, number] | null; onOpenDetail: () => void }) {
  ...
  const { startDate, endDate } = computeSpotDateRange(spot)
  const dateLabel = getDateDisplay(spot.scheduleNote, startDate, endDate, spot.specificDates)
